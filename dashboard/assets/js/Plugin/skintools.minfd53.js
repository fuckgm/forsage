...
! function(global, factory) {
 ...
 if (window.localStorage) {
  var getLevel = function(url, tag) {
   ...
   settings = localStorage.getItem("remark.material.iconbar.skinTools");
  if (settings) {
   if ("{" === settings[0] && (settings = JSON.parse(settings)), settings.primary && "primary" !== settings.primary) {
    var head = document.head,
     link = document.createElement("link");
    link.type = "text/css", link.rel = "stylesheet", link.href = getLevel(window.location.pathname, "iconbar") + "assets/skins/" + settings.primary + ".css", link.id = "skinStyle", head.appendChild(link)
   }
  ...
  document.addEventListener && document.addEventListener("DOMContentLoaded", function() {
   ...
   ({
    ...
    storageKey: "remark.material.iconbar.skinTools",
    defaultSettings: {
     sidebar: "light",
     navbar: "primary",
     navbarInverse: "true",
